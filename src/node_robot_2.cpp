...
bool Example::add(mini_multi_agent::task_srv::Request &req,
	...
	if (req.seq==0){
		ROS_INFO("Robot 2 , just have a rest , we will going to work in 3 seconds . good luck!");
		return true;
	}
	stas = 1;
	if(stas==0)
	...
	ROS_INFO("Robot 2 - task id : [%d] comming , status change to [%s]" , req.seq, res.status.c_str());
	return true;
}
...
Example::Example(int seconds):seconds(seconds) {
	...
	ros::ServiceServer service = nh.advertiseService("agent_task_2" ,&Example::add,this);
